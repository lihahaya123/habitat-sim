...
| 4 | other | `(255, 190, 60)` | `#FFBE3C` | 其他已识别语义物体，黄色/橙色 |
| 5 | unknown | `(20, 20, 20)` | `#141414` | 未知或无可靠真值区域，近黑色 |

颜色只用于 PNG 可视化，训练标签仍以 `bev_masks/*.npy` 中的六通道 0/1 数组为准。由于标签是 multi-hot，同一网格可能属于多个类别；生成单张彩色图时按 `obstacle > other > wall > carpet > floor > unknown` 的顺序显示，高优先级类别会覆盖低优先级颜色。
